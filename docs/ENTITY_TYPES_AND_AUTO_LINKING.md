...
```markdown
Sarah has also worked closely with [[companies/acme]] on the API migration.
```
...
automatically. If the body also mentions `[[companies/acme]]`, that reference
is preserved as a narrative mention and can still be upgraded by the extractor
when the surrounding prose is strong enough.

## Verification

1. Create a person page with `company: companies/acme`.
