...
describe('knobsHash determinism + cross-mode separation (CDX-4)', () => {
  ...
  test('KNOBS_HASH_VERSION constant exposed for migrations to bump on schema change', () => {
    ...
    // issue #1 (F14b): bumped 11→12 to fold the retrieval policy itself
    // (hard-exclude prefixes hx= + source-boost map sb=) into the hash —
    // unlike the #1777 bump, which invalidated rows for ONE policy change,
    // v=12 makes ALL future exclude/boost changes self-invalidating.
...
describe('v0.42.3.0 — autocut knobs', () => {
  test('KNOBS_HASH_VERSION is 12 (11→12 retrieval-policy fold, issue #1)', () => {
    expect(KNOBS_HASH_VERSION).toBe(12);
  });
