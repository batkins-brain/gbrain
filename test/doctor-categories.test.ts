 ...
 *
 * Reads doctor-owned source files via a literal-string scan, enumerates every
 * `name: '<...>'` Check name, and asserts each appears in exactly ONE
...
describe('doctor-categories drift guard', () => {
  test('every check name in doctor-owned source belongs to exactly one category set', () => {
    ...
    if (missing.length > 0) {
      throw new Error(
        `These check names appear in doctor-owned source but are not categorized in ` +
  ...
  test('every categorized name is currently used in doctor-owned source (no stale entries)', () => {
    const discovered = enumerateCheckNames();
    const allCategorized = new Set<string>([
      ...BRAIN_CHECK_NAMES,
      ...SKILL_CHECK_NAMES,
    ...
    if (stale.length > 2) {
      throw new Error(
        `These categorized names no longer appear in doctor-owned source: ${stale.sort().join(', ')}. ` +
