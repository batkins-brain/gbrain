...
- [ ] **v0.42.57.0-followup: `gbrain doctor` check for case-shadow paths left by the pre-fix write-back.** v0.42.57.0 stopped `resolveSlugPathOnDisk` from creating lowercase twins of capitalized notes, but it does NOT clean up twins an older gbrain already wrote. Those files are untracked in the brain repo, so `git clean -fd` silently deletes them, and a case-insensitive mirror (macOS/Windows Obsidian) collides on them instead. The CHANGELOG ships a `find | awk` one-liner users can run by hand; a doctor check should do it natively: walk the source's `local_path`, group entries by lowercased path, and report any group with 2+ distinct spellings, naming the tracked note and the shadow. Facts in a shadow file are also indexed in the `facts` table, so the remediation is "fold the `## Facts` block into the correctly-named note, delete the shadow", which the check can print. Note the ordering trap when testing: while a shadow exists, `resolveSlugPathOnDisk` resolves ONTO it (exact match wins by design), so a fix-verification fixture has to clear the shadow first. Priority: P3 (correct-by-default now; this is cleanup for brains that ran the old code). Related class: `canonicalLookup` case-sensitivity above.
